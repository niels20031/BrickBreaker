...
class Game:
    ...
    def update_high_score(self):
        ...
        if getattr(self, 'infinite_mode', False):
            if self.score > self.high_score_data.get("infinite_score", 0):
                ...
                self.high_score_data["infinite_date"] = datetime.now().strftime("%Y-%m-%d %H:%M")
            ...
            if self.score > self.high_score_data.get("score", 0):
                ...
                self.high_score_data["date"] = datetime.now().strftime("%Y-%m-%d %H:%M")
                self.save_high_score()
    ...
    def create_level(self, level):
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                color = [GREEN, YELLOW, RED][min(row // (max(1, rows // 3)), 2)]
        ...
        if self.state == GameState.PLAYING and self.game_active:
                    ...
                    if ball.rect.colliderect(brick.rect):
                        ...
                        if brick.is_destroyed():
                            ...
                            if random.random() < 0.15:  # 15% kans
                                powerup_type = random.choice([PowerupType.WIDER_PADDLE, PowerupType.MULTI_BALL, PowerupType.SLOW_BALL])
            ...
            if len(self.bal_beheer.balls) == 0:
                ...
                if self.lives <= 0:
                    ...
                    new_ball = Bal(int(self.peddel.rect.centerx), SCREEN_HEIGHT - 80)
    ...
    def draw_menu(self):
        ...
        high_score_text = self.font_small.render(f"Top Score: {self.high_score_data.get('score',0)}", True, YELLOW)
        self.screen.blit(high_score_text, ((SCREEN_WIDTH - high_score_text.get_width()) // 2, 240))

        infinite_text = self.font_small.render(f"Oneindige Top: {self.high_score_data.get('infinite_score',0)}", True, LIGHT_BLUE)
        self.screen.blit(infinite_text, ((SCREEN_WIDTH - infinite_text.get_width()) // 2, 280))
        
        start_text = self.font_medium.render("Druk SPATIE om Oneindige Modus te starten", True, LIGHT_BLUE)
        self.screen.blit(start_text, ((SCREEN_WIDTH - start_text.get_width()) // 2, 320))
        
        
        
        quit_text = self.font_tiny.render("Druk ESC om af te sluiten", True, GRAY)
        self.screen.blit(quit_text, ((SCREEN_WIDTH - quit_text.get_width()) // 2, 500))
    ...
    def draw_level_select(self):
        ...
        for level in range(1, 6):
            if level <= 3:
                ...
                level_text = self.font_medium.render(f"Niveau {level}", True, color)
                self.screen.blit(level_text, (x - level_text.get_width() // 2, y))
        
        level_3_text = self.font_medium.render("Niveau 3", True, GRAY)
        self.screen.blit(level_3_text, ((SCREEN_WIDTH - level_3_text.get_width()) // 2, 400))
        
        instructions = self.font_small.render("Druk 1, 2, of 3 om te starten", True, LIGHT_BLUE)
        self.screen.blit(instructions, ((SCREEN_WIDTH - instructions.get_width()) // 2, 550))
    ...
    def draw_game(self):
        ...
        # HUD
        level_text = self.font_small.render(f"Niveau: {self.current_level}", True, WHITE)
        ...
        # Show high score
        high_score_text = self.font_tiny.render(f"Top Score: {self.high_score_data['score']}", True, LIGHT_BLUE)
        self.screen.blit(high_score_text, (20, 70))
        
        # Show ball count if multiple balls
        if len(self.bal_beheer.balls) > 1:
            balls_text = self.font_tiny.render(f"Ballen: {len(self.bal_beheer.balls)}", True, LIGHT_BLUE)
        ...
        if self.peddel.wider_duration > 0:
            ...
            self.screen.blit(boost_text, ((SCREEN_WIDTH - boost_text.get_width()) // 2, 70))
        
        # Show slow ball status
        if self.slow_ball_active:
            slow_text = self.font_tiny.render("TRAAGMODUS", True, PURPLE)
            self.screen.blit(slow_text, (SCREEN_WIDTH - slow_text.get_width() - 20, 70))
        
        if len(self.bal_beheer.balls) > 0 and not self.bal_beheer.balls[0].launched:
            launch_text = self.font_tiny.render("Druk SPATIE om te lanceren", True, LIGHT_BLUE)
            self.screen.blit(launch_text, ((SCREEN_WIDTH - launch_text.get_width()) // 2, SCREEN_HEIGHT - 30))
    
    def draw_level_complete(self):
        complete_text = self.font_large.render("NIVEAU VOLTOOID!", True, GREEN)
        self.screen.blit(complete_text, ((SCREEN_WIDTH - complete_text.get_width()) // 2, 150))
        
        score_text = self.font_medium.render(f"Score: {self.score}", True, YELLOW)
        self.screen.blit(score_text, ((SCREEN_WIDTH - score_text.get_width()) // 2, 300))
        
        if self.current_level >= 5:
            next_text = self.font_small.render("Je hebt alle niveaus verslagen!", True, LIGHT_BLUE)
        else:
            next_text = self.font_small.render(f"Volgende: Niveau {self.current_level + 1}", True, LIGHT_BLUE)
        self.screen.blit(next_text, ((SCREEN_WIDTH - next_text.get_width()) // 2, 400))
        
        continue_text = self.font_small.render("Druk SPATIE om door te gaan", True, YELLOW)
        self.screen.blit(continue_text, ((SCREEN_WIDTH - continue_text.get_width()) // 2, 500))
    
    def draw_game_over(self):
        game_over_text = self.font_large.render("GAME OVER", True, RED)
        self.screen.blit(game_over_text, ((SCREEN_WIDTH - game_over_text.get_width()) // 2, 150))
        
        score_text = self.font_medium.render(f"Uiteindelijke Score: {self.score}", True, YELLOW)
        self.screen.blit(score_text, ((SCREEN_WIDTH - score_text.get_width()) // 2, 300))
        
        level_text = self.font_small.render(f"Niveau bereikt: {self.current_level}", True, LIGHT_BLUE)
        self.screen.blit(level_text, ((SCREEN_WIDTH - level_text.get_width()) // 2, 400))
        
        restart_text = self.font_small.render("Druk SPATIE om terug naar menu", True, YELLOW)
        self.screen.blit(restart_text, ((SCREEN_WIDTH - restart_text.get_width()) // 2, 500))
